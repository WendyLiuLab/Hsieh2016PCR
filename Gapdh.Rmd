...
cqs = read_csv("raw-data/gapdh/2016-09-04 gapdh -  Quantification Cq Results_0.csv") %>%
  select(Well, Cq) %>%
  inner_join(annotations())

melts = read_csv("raw-data/gapdh/2016-09-04 gapdh -  Melt Curve Derivative Results_SYBR.csv") %>%
  select(-X1) %>%
